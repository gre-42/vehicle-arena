...
#include <VehicleArena/Images/Coordinates.hpp>
...
TransformationMatrix<float, float, 2> VA::intrinsic_matrix_from_dimensions(
    float focal_length,
...
static float isin(int n) {
    return float(VA::sign(n)) * (float((std::abs(n) % 4) == 1) - float((std::abs(n) % 4) == 3));
}

TransformationMatrix<float, float, 2> VA::rotated_intrinsic_matrix(
...
FixedArray<float, 4, 4> VA::cv_to_opengl_hz_intrinsic_matrix(
    const TransformationMatrix<float, float, 2>& intrinsic_matrix,
    float width,
    float height,
    float z_near,
...
FixedArray<float, 3> VA::cv_to_opengl_coordinates(const FixedArray<float, 3>& p) {
    return FixedArray<float, 3>{ p(0), -p(1), -p(2) };
}

TransformationMatrix<float, float, 3> VA::cv_to_opengl_extrinsic_matrix(
    const TransformationMatrix<float, float, 3>& extrinsic_matrix)
{
    return cv_to_opengl_matrix() * extrinsic_matrix * opengl_to_cv_matrix();
}

TransformationMatrix<float, float, 3> VA::opengl_to_cv_extrinsic_matrix(
    const TransformationMatrix<float, float, 3>& extrinsic_matrix)
{
    return cv_to_opengl_extrinsic_matrix(extrinsic_matrix);
}

TransformationMatrix<float, float, 3> VA::cv_to_opengl_matrix() {
...
TransformationMatrix<float, float, 3> VA::opengl_to_cv_matrix() {
