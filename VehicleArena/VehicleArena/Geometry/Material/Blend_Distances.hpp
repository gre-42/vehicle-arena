...
class SquaredStepDistances {
    friend struct std::hash<VA::SquaredStepDistances>;
public:
...
template <>
struct std::hash<VA::SquaredStepDistances>
{
    std::size_t operator() (const VA::SquaredStepDistances& a) const {
        return VA::fixed_array_hash(a.distances2_);
    }
};
