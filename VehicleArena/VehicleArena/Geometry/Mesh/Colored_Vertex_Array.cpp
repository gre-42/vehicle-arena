...
#include <VehicleArena/Geometry/Delaunay.hpp>
...
#include <VehicleArena/Geometry/Primitives/Line_3D.hpp>
#include <VehicleArena/Geometry/Primitives/Polygon_3D.hpp>
    ...
    , bounding_sphere_has_value_{ false }
{
    ...
    if (!any(morphology.physics_material & (PhysicsMaterial::ATTR_VISIBLE | PhysicsMaterial::ATTR_COLLIDE))) {
        throw std::runtime_error("Mesh \"" + this->name.full_name() + "\" is neither visible nor collidable");
    }
...
const ExtremalAxisAlignedBoundingBox<TPos, 3>& ColoredVertexArray<TPos>::aabb() const {
    ...
    auto vs = vertices();
    if (vs.empty()) {
        throw std::runtime_error("Cannot compute AABB of \"" + name.full_name() + "\" because it has no vertices");
    }
...
const ExtremalBoundingSphere<TPos, 3>& ColoredVertexArray<TPos>::bounding_sphere() const {
    ...
    auto vs = vertices();
    if (vs.empty()) {
        throw std::runtime_error("Cannot compute bounding sphere of \"" + name.full_name() + "\" because it has no vertices");
    }
...
TPos ColoredVertexArray<TPos>::radius(const FixedArray<TPos, 3>& center) const {
    auto vs = vertices();
    if (vs.empty()) {
        throw std::runtime_error("Mesh \"" + name.full_name() + "\" has no vertices");
    }
...
template class VA::ColoredVertexArray<float>;
template class VA::ColoredVertexArray<CompressedScenePos>;
