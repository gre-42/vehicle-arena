...
#pragma once
#include <VehicleArena/Math/Round.hpp>
#include <cmath>
#include <concepts>
#include <concepts>
#include <cstdint>
...
public:
    ...
    inline constexpr explicit FixedPointNumber(const std::floating_point auto& value)
        : count{ static_cast<TInt>(VA::round(value * denominator)) }
    {}
...
namespace std {
    template <std::integral TInt, std::intmax_t denominator>
    class numeric_limits<VA::FixedPointNumber<TInt, denominator>> {
        using T = VA::FixedPointNumber<TInt, denominator>;
    public:
    ...
    template <std::integral TInt, std::intmax_t denominator>
    std::string to_string(const VA::FixedPointNumber<TInt, denominator>& v) {
