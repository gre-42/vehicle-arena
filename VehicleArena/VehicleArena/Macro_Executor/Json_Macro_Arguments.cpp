...
std::string JsonMacroArguments::path(std::string_view name) const {
    ...
    if (res.is_variable) {
        throw std::runtime_error('"' + std::string{ name } + "\" is a variable, not a path");
    }
    return res.path;
}
...
std::vector<JsonMacroArguments> JsonMacroArguments::children(std::string_view name) const {
    ...
    return VA::get_vector<nlohmann::json>(el, [this](const nlohmann::json& c){return as_child(c);});
}
