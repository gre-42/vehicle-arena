...
#include <VehicleArena/Regex/Match_Counter.hpp>
    ...
    const nlohmann::json& block)
{
    ...
    if (nfound > 1) {
        throw std::runtime_error("Found variable in multiple dictionaries (globals, locals, block): \"" + std::string{ s } + '"');
    }
    if (git != globals.end()) {
        return *git;
    }
    ...
    size_t recursion)
{
    ...
    subst = [&subst_](const std::string_view& s){return VA::substitute_dollar(s, subst_);};
