...
#include <VehicleArena/FPath.hpp>
#include <VehicleArena/Macro_Executor/Boolean_Expression.hpp>
#include <VehicleArena/Macro_Executor/Json_Expression.hpp>
#include <VehicleArena/Macro_Executor/Macro_Keys.hpp>
#include <VehicleArena/Macro_Executor/Macro_Recorder.hpp>
...
#include <VehicleArena/Time/Fps/Object_Life_Time.hpp>
#include <filesystem>
#include <fstream>
#include <iostream>
#include <stdexcept>
...
public:
    ...
    std::string spath(const std::filesystem::path& f) const {
        if (f.empty()) {
            ...
            throw std::runtime_error("Could not find relative path \"" + f.string() + "\" in script directory or search directories. Script: \"" + script_filename_ + '"');
        }
    }
private:
    ...
    if (j.type() == nlohmann::detail::value_t::object) {
        ...
        if (include) {
            ...
            if (auto c = jv.try_at<std::string>(MacroKeys::context); c.has_value()) {
                context = VA::eval<std::string>(*c, global_args, merged_args, asset_references_);
            } else {
                context = context_;
            }
...
nlohmann::json MacroLineExecutor::eval(const std::string& expression, const JsonView& variables) const {
    auto global_args = global_json_macro_arguments_.json_macro_arguments();
    return VA::eval(expression, global_args, variables, asset_references_);
}

nlohmann::json MacroLineExecutor::eval(const std::string& expression) const {
    auto global_args = global_json_macro_arguments_.json_macro_arguments();
    return VA::eval(expression, global_args, asset_references_);
}

template <class T>
T MacroLineExecutor::eval(const std::string& expression, const JsonView& variables) const {
    auto global_args = global_json_macro_arguments_.json_macro_arguments();
    return VA::eval<T>(expression, global_args, variables, block_arguments(), asset_references_);
}

template <class T>
T MacroLineExecutor::eval(const std::string& expression) const {
    auto global_args = global_json_macro_arguments_.json_macro_arguments();
    return VA::eval<T>(expression, global_args, block_arguments(), asset_references_);
}
