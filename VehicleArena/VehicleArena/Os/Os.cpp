...
#ifdef __ANDROID__
#include <VehicleArena/Android/ndk_helper/AUi.hpp>
#include <VehicleArena/Android/ndk_helper/NDKHelper.h>
#else
#include <VehicleArena/Os/Set_Thread_Name_Native.hpp>
#include <iostream>
#endif
...
LogLevel VA::log_level_from_string(const std::string& s) {
...
void VA::set_log_level(LogLevel log_level) {
...
std::unique_ptr<std::istream> VA::create_ifstream(
...
std::vector<uint8_t> VA::read_file_bytes(const std::filesystem::path& filename) {
    return AUi::ReadFile(filename);
}

std::unique_ptr<std::ostream> VA::create_ofstream(
    const std::filesystem::path& filename,
    std::ios_base::openmode mode,
    FileStorageType storage_type)
{
...
bool VA::path_exists(const std::filesystem::path& filename) {
...
ndk_helper::DirectoryIterator VA::list_dir(const std::filesystem::path& path) {
    return AUi::ListDir(path.c_str());
}

bool VA::is_listable(const ndk_helper::DirectoryEntry& entry) {
    return entry.is_listable();
}

void VA::set_thread_name(const std::string& name) {
...
bool VA::path_exists(const std::filesystem::path& path) {
    std::error_code ec;
...
std::filesystem::directory_iterator VA::list_dir(const std::filesystem::path& path) {
    return std::filesystem::directory_iterator(path);
}

bool VA::is_listable(const std::filesystem::directory_entry& entry) {
    std::error_code ec;
...
void VA::verbose_abort(const std::string& message) {
