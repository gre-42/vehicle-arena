...
#include <VehicleArena/Geometry/Arbitrary_Orthogonal.hpp>
...
#include <VehicleArena/Physics/Collision/Resolve/Handle_Tire_Triangle_Intersection.hpp>
#include <VehicleArena/Physics/Rigid_Body/Attached_Wheel.hpp>
...
void FrictionContactInfo1::solve(float dt, float relaxation, size_t iteration, size_t niterations) {
    ...
    if (float vl2 = sum(squared(v3)); vl2 > 1e-12) {
        ...
        if (!any(VA::isnan(clamping_direction_))) {
...
std::ostream& VA::operator << (std::ostream& ostr, const FrictionContactInfo1& fci1) {
...
void VA::solve_contacts(std::list<std::unique_ptr<IContactInfo>>& cis, float dt) {
